...
import cn.edu.bjfu.igarden.rec_engin.TFIDFCalculator;
...
public class zuofa {
    ...
    private PlantService plantService=applicationContext.getBean(PlantService.class);
    ...
    public  ArrayList<String> getUserWord(int id){//得到用户的分词结果
        ...
        for (int i = 0; i < userLikeList.size(); i++) {
            res=sw.getFilterWord(answerMapper.getColumnWithId2(userLikeList.get(i)),expectedNature);
