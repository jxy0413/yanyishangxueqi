package cn.edu.bjfu.igarden.rec_engin.handle;


import cn.edu.bjfu.igarden.dao.AnswerMapper;
import cn.edu.bjfu.igarden.rec_engin.*;
import cn.edu.bjfu.igarden.util.SpringUtils;
...
public class mingcheng {
    ...
    private AnswerMapper answerMapper=applicationContext.getBean(AnswerMapper.class);
     ...
     */
    public  Map<Integer,Map<String,Double>> tfidfmap=new HashMap<Integer, Map<String, Double>>();

    {
        ...
        res = answerMapper.getColumn2();
        for (int i = 0; i < res.size(); i++) {
            ArrayList<String> document = new ArrayList<String>();
            if (res.get(i) != null) {
                document = sw.getFilterWord(res.get(i), expectedNature);
            } else document.add(null);
            map.put(Integer.parseInt(answerMapper.idlist().get(i).toString()), document);
            docs.add(document);
        }
    }
    ...
    public  ArrayList<String> getUserWord(int id){//得到用户的分词结果
        ArrayList<String> res=new ArrayList<String>();
        split sw=new split();
        ArrayList<Integer> userLikeList=answerMapper.queryLike(id);
        ArrayList<String> doc=new ArrayList<String>();
        for (int i = 0; i < userLikeList.size(); i++) {
            res=sw.getFilterWord(answerMapper.getColumnWithId(userLikeList.get(i)),expectedNature);
    ...
    public  Map<String,Double> getUserTFIDF(int id){//得到用户的词的tfidf结果
        ArrayList<String> doc=new ArrayList<String>();
        doc=getUserWord(id);
        return getTFIDF(doc);
    }
    public  void getRecipeTFIDF(){//得到所有菜谱的tf idf数据
        ArrayList<String> doc=new ArrayList<String>();
     ...
     */
    public   Map<Integer,Double>  getSimList1(int id){
        long startTime = System.currentTimeMillis();   //获取开始时间
