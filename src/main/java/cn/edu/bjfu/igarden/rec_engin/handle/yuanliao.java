...
//        }};
        ...
        Map<String, double[]> vec1=plantService.getUserVector(id,expectedNature,filter);
        Map<Integer,Double> simList = new HashMap<Integer,Double>();//存放计算的相似值

        for (int i = 0; i < answerMapper.idlist().size(); i++) {//在这里修改总共对比的条数
            if(!(userLikeList.contains(answerMapper.idlist().get(i)))){//去掉已经标记过喜欢的菜谱。
                 ...
                 */
                Map<String, double[]> vec2=plantService.getRecipeVector(i,expectedNature,filter);
                vecSpa=mix.mapMix(vec1,vec2);
                double res=sim.countCos(vecSpa);
                if(!Double.isNaN(res)) {
                    simList.put((Integer) answerMapper.idlist().get(i),res);
                }
                else simList.put((Integer) answerMapper.idlist().get(i),0.0);
